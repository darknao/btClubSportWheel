...
// delayMicroseconds(MAX_SPEED - delta_t);
    ...
    if (timout > max_delay || (in_changed && timout > 10000))
    {
      ...
      if (rotary_debounce != 0)
        Serial.println(String("RESET!!!!!!!!!ROTARY : ") + rotary_value);
      if (in_changed)
  ...
  while ((result = WT12.read()) < 256 && !got_hid)
